    ...
    fn try_from(value: &str) -> Result<Self, Self::Error> {
        lazy_static! {
            static ref RE: Regex = Regex::new(
                "^([[:xdigit:]]{4})\\.([[:xdigit:]]{4})\\.([[:xdigit:]]{4})\\.([[:xdigit:]]{4})$").unwrap();
        }

        if let Some(c) = RE.captures(value) {
            Ok(Self((capture2u64(&c, 1) << 48) | (capture2u64(&c, 2) << 32)
                | (capture2u64(&c, 3) << 16) | capture2u64(&c, 4)))
        }
        else {
...
impl Shl for Label64 {
    type Output = Self;
    fn shl(self, rhs: Self) -> Self { Self(self.0 << rhs.0) }
}

impl Shr for Label64 {
    type Output = Self;
    fn shr(self, rhs: Self) -> Self { Self(self.0 >> rhs.0) }
}
...
mod tests {
    ...
    fn l64(v: u64) -> Label64 { Label64::new(v) }
    ...
    fn l64_from_string() {
        ...
        assert_eq!(Label64::try_from("0000.0003.64b5.10e5").unwrap(), l64(14574489829));
        assert_eq!(Label64::try_from("0002.0003.64b5.10e5").unwrap(), l64(562964527911141u64));
